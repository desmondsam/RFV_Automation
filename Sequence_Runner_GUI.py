...
import Test_Sequencer_TXInBand
import logging
...
def run_sequencer(log_file_loc,spreadsheet_loc,sig_gen_ip,sig_analyzer_ip,master_switch_ip,slave_switch_ip,pwr_supp_ip,chamber_ip,band,tGDelay,tGLength,trigSource,loops,band_edges,switch_attenuation,cal_file_loc,cal_file_name,sequences,pipe_list,channels,config_dl,refLvl,temps,screenshot_loc,init_instruments, test_info,supp_volt,pwr_up_time, stat_bar):
    ...
    try:
        runner = Test_Sequencer_TXInBand.TestSequence(
        ...
        stat_bar.info('Running Tets. Please wait for execution to be complete!')
        ...
        stat_bar.error('Unable to Connect to one or more Instruments. Please Check IPs')
...
def config_sidebar():
    with st.sidebar:
        ...
        sequencer = st.radio('Select Sequencer', ['TX Sequencer', 'RX Sequencer'], help='Select the Test Sequence you want to run')
...
def config_ant_dup_exp(antenna_config):
    ...
    num_of_pipes = ac_c1.selectbox('Number of Pipes', [32, 64], index=1, help = 'Select the total number of antennas on the RU')
    dup_sel = ac_c1.selectbox('Duplexing', ['TDD', 'FDD'], help='Pick the Duplexing of choice')
    pipes_selector = ac_c2.radio('Antenna Selector', ['Single', 'All', 'Hand-Pick', 'Range'], help = 'Select between measuring Individual  [vs]  All  [vs]  Hand-Picked  [vs]  Sequential(Range) Antenna(s)')
    if(pipes_selector == 'All'):
        pipe_sel = range(1, num_of_pipes+1)
    elif(pipes_selector == 'Hand-Pick'):
...
def main():
    ...
    # main_body = get_main_container(title='PARRoT', subheader='Python Automated RF Radio Tests')
    ...
    if(sequencer == 'TX Sequencer'):
        ...
        bw = carr_config.selectbox('Carrier Bandwidth', bw_list, index=12)
        ...
        test_info = {
            'Bench_ID' : bench_info,
            'RU_Name' : radio_name,
            'RU_Build' : radio_build,
            'RU_Serial_Num' : radio_serial,
            'RU_SW' : radio_sw,
            'DC_Voltage' : supp_volt,
            'Tester_Comment': tester_comm,
            'Tester' : tester,
        }
        got_seq_inps = seq_form.form_submit_button('Run Test')
        ...
        if(got_seq_inps):
            ...
            ###Running
            # ip_addr_list = [sig_analyzer_ip, sig_gen_ip, pwr_supp_ip, master_switch_ip, slave_switch_ip]
            # for ip_addr in [sig_analyzer_ip, sig_gen_ip, pwr_supp_ip, master_switch_ip, slave_switch_ip]:
            # ip_validity = [General_Utils.validate_ip_address(ip_addr) for ip_addr in ip_addr_list]
            # if(False in ip_validity):
                # st.error('Check IPs. One or more IP(s) are invalid')
            # else:
                # st.success('IPs are Valid')
            # if(valid_ip):
            #     st.success('IPs are Valid')
            # else:
            #     st.warning('Check IPs. An IP is invalid')
            
            run_sequencer(log_file_loc,spreadsheet_loc,sig_gen_ip,sig_analyzer_ip,master_switch_ip,slave_switch_ip,pwr_supp_ip,chamber_ip,band,tGDelay,tGLength,trigSource,loops,band_edges,switch_attenuation,cal_file_loc,cal_file_name,sequences,pipe_list,channels,config_dl,refLvl,temps,screenshot_loc,init_instruments,test_info,supp_volt,pwr_up_time,status_bar)
            ###########################################################
            # st.write(dup_sel)
            # st.write(pipe_list)
