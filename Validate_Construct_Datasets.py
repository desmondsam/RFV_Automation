...
def get_acp_dataset(acp_res, pipe, freq, loop, bw, pwr, temp, tm, noc, sig_file_name, channel):
        ...
        pwr_res = generate_dataset(loop, pipe, freq, channel, 'Output Power', 'Channel Power', opSmin, opSmax, acp_res[0], 'dBm', status, temp, tm, noc, bw, sig_file_name, tc_num='6.2')
...
def construct_acp_dataset(offs, index, acp_res, pipe, freq, loop, bw, temp, tm, noc, sig_file_name, channel):
    ...
    return generate_dataset(loop, pipe, freq, channel, 'ACLR', measName, spec.spec_min, spec.spec_max, res, 'dBc', status, temp, tm, noc, bw, sig_file_name, tc_num='6.6.3')
...
def get_sem_dataset(pipe_obue, pipe, frequency, loop, bw, temp, tm, noc, sig_file_name, channel, tName='OBUE'):
    ...
    for ind in range(len(pipe_obue)):
        for i in [0, 1]:
            ...
            status = validate_data(res, spec.spec_min, spec.spec_max)
            sem_datasets.append(generate_dataset(loop, pipe, frequency, channel, tName, measName, spec.spec_min, spec.spec_max, res, unit, status, temp, tm, noc, bw, sig_file_name, pk_freq, rbw, tc_num='6.6.4'))
    return sem_datasets
    ...
    for carr_evm in pipe_evm:
        for ind in range(1, len(carr_evm)):
            ...
            if(ind == 1):
                ...
                unit = 'dBm'
                tc_num = '6.3.3'
            evm_datasets.append(generate_dataset(loop, pipe, frequency, channel, tName, measName, spec.spec_min, spec.spec_max, res, unit, status, temp, tm, noc, bw, sig_file_name, tc_num=tc_num))
    return evm_datasets
...
def get_oop_dataset(pipe_oop, pipe, frequency, loop, bw, temp, tm, noc, sig_file_name, channel):
    ...
    for ind in range(len(pipe_oop)):
        ...
        if ind in [0, 3]:
            ...
            spec = Specs(None, None)
            status = None
        oop_datasets.append(generate_dataset(loop, pipe, frequency, channel, tName, measName, spec.spec_min, spec.spec_max, res, unit, status, temp, tm, noc, bw, sig_file_name, tc_num='6.4'))
    return oop_datasets
...
def generate_dataset(sequence, pipe, freq, channel, testName, measName, spec_min, spec_max, res, unit, status, temp, tm, noc, carrBw, sig_file_name, pk_freq=None, rbw=None, tc_num=None):
    return DataSet(
...
def main():
    ...
    a = get_acp_dataset([-6.64, -66.45, -66.92, -67.12, -68.58], 1, 3605, 1, 20, -5, 25, '3_1a', 1, 'ABC', 'Mid')
    print(a)
